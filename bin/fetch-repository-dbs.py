...
def index_db(name, tempdb):
    ...
    if tempdb.endswith("primary.sqlite"):
        ...
        for package_info in conn.execute("SELECT * FROM packages"):
            ...
            conn.execute(
                "UPDATE packages SET rpm_sourcerpm_name = ? WHERE pkgKey = ?",
                [nevra[0].name, package_info["pkgKey"]],
            )
        conn.commit()
        conn.close()
...
def get_repository_urls_for(product, version):
    ...
    if product == "fedora":
        ...
        return [
            ...
            (
                "{}/pub/fedora/linux/updates/{}/Everything/x86_64/repodata".format(
                    MIRROR, version
                ),
                release + "-updates",
            ),
            (
                "{}/pub/fedora/linux/updates/testing/{}/Everything/x86_64/repodata".format(
                    MIRROR, version
                ),
        ...
        return [
            ...
            (
                "{}/pub/epel/testing/{}/x86_64/repodata".format(MIRROR, version),
                release + "-testing",
            ),
        ]
    elif product == "epel" and int(version) >= 8:
        return [
            (
                "{}/pub/epel/{}/Everything/x86_64/repodata/".format(MIRROR, version),
                release,
            ),
            (
                "{}/pub/epel/testing/{}/Everything/x86_64/repodata".format(
                    MIRROR, version
                ),
                release + "-testing",
            ),
...
def main():
    ...
    if r.status_code != 200:
        sys.exit(
            "Failed to fetch active releases from PDC (request returned {})".format(
                r.status_code
            )
        )
